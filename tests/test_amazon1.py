...
@pytest.mark.test1
def test_get_started_link(page:Page, main_page):
...
@pytest.mark.test2       
def test_nutella(page:Page, main_page):
    search_word = ""
...
@pytest.mark.test3
def test_footer(page:Page, main_page):
    
    page.mouse.wheel(0,13188)
    page.wait_for_timeout(5000)
    for i in range(0,12):
        expect(page.locator(Amazon_MainPage.FOOTER_REDUZIERTE_BWARE).nth(i)).to_be_visible()
